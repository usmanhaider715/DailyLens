...
{
  "headline": "Rewritten SEO-optimized headline, punchy and clear, max 12 words",
...
export async function processBatch(articles, concurrency = 3) {
  const results = [];
  let idx = 0;
  ...
  async function worker() {
    while (idx < articles.length) {
      ...
      try {
        const parsed = await processArticle(raw);
