...
export function normalizeSeoArticleOutput(parsed, raw = {}) {
  ...
  let headline = stripHtml(parsed.headline || parsed.title || raw.title || 'Untitled')
    .replace(/\s+/g, ' ')
    .trim();
  headline = truncateAtWord(headline, 60);

  let summary = stripHtml(parsed.summary || parsed.excerpt || '')
    .replace(/\s+/g, ' ')
    .trim();
  ...
  return {
    ...
    readTime: parsed.readTime || estimateReadMinutes(parsed.body),
  };
}
